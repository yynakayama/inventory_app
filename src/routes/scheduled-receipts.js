...
router.get('/', authenticateToken, requireReadAccess, async (req, res) => {
  ...
  try {
    ...
    console.error('予定入荷一覧取得エラー:', error.message);
    res.status(500).json({
      success: false,
...
router.post('/', authenticateToken, requireMaterialAccess, async (req, res) => {
  ...
  try {
    ...
    console.error('発注登録エラー:', error.message);
    res.status(500).json({
      success: false,
      message: '発注登録に失敗しました',
      error: error.message
...
router.put('/:id/delivery-response', authenticateToken, requireMaterialAccess, async (req, res) => {
  ...
  try {
    ...
    console.error('納期回答登録エラー:', error.message);
    res.status(500).json({
      success: false,
...
router.put('/:id/mark-received', authenticateToken, requireMaterialAccess, async (req, res) => {
  ...
  try {
    ...
    console.error('入荷実績処理エラー:', error.message);
    res.status(500).json({
      success: false,
...
router.put('/:id/cancel', authenticateToken, requireMaterialAccess, async (req, res) => {
  ...
  try {
    ...
    console.error('発注キャンセルエラー:', error.message);
    res.status(500).json({
      success: false,
...
router.get('/:id', authenticateToken, requireReadAccess, async (req, res) => {
  ...
  try {
    ...
    console.error('入荷予定詳細取得エラー:', error.message);
    res.status(500).json({
      success: false,
